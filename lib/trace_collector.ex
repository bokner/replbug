defmodule TraceCollector do
  ...
  In addition, TraceCollector allows to materialize traces as a variable, and then analyze the call data
  ...
  Example 1:
   Trace DateTime.utc_now/0. Let the tracer run for 60 secs or until 10 trace messages are emitted.
    iex(1)> TraceCollector.start("DateTime.utc_now/0", time: 60_000, msgs: 10)
    ...
    iex(3)> call_traces = TraceCollector.stop
    %{
    #PID<0.1392.0> => [
      %{
        args: [],
  ...
  iex(1)> TraceCollector.start("DateTime", "String.contains?/2"], time: 60_000, msgs: 100)
  ...
  iex(4)> calls = TraceCollector.stop |> TraceCollector.calls()
  ...
  def start(call_pattern, opts \\ []) do
    ...
    call_pattern
    |> add_return_opt()
    |> create_call_collector(
      Keyword.put(opts, :print_fun, print_fun)
    )
  end
  ...
  defp add_return_opt(call_pattern) when is_binary(call_pattern) do
    ...
    case String.split(call_pattern, ~r{::}, trim: true, include_captures: true) do
      [no_opts_call] -> "#{no_opts_call} :: return"
      [call, "::", opts] ->
  ...
  defp extract_trace_data(%Rexbug.Printing.Call{
         mfa: mfa,
         from_pid: caller_pid,
         from_mfa: parent,
         time: rexbug_time
...
defmodule TraceCollector.Server do
  @behaviour GenServer
  ...
  def handle_info({:DOWN, _ref, :process, _pid, _reason}, state) do
    ...
    The tracing has been completed. Use:
      TraceCollector.stop to get the trace records into your shell.
    ''')
